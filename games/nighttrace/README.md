...
- `v1.20.0` — campaign presentation and combat-readability milestone: reliable zero-lag audio and cinematic startup, all eleven story scenes in Combat Lab, expressive dialogue portraits, source-aware hero damage feedback with compact color-coded combat text, brighter grounded hostile boundaries, an explicit and test-covered forgiving hero hitbox, and the Combat Lab-only **Dawnward Aegis** prototype with ranks I–V plus an Awakened state
- `v1.20.1` — corrective story/combat release: every Memory is subtitle-only with complete dialogue and no voice-reel bleed, interlude actor clutter is removed, Level clears gain a durable Sovereign-defeated transition, every hostile field/lane/destination keeps a brighter broken filament boundary through its final dodge frame, the hero uses a model-aligned body hitbox, and Dawnward Aegis is visible at the top of Combat Lab with Rank I active by default
- `v1.20.2` — cinematic and combat-fidelity release: the Prologue duplicate runtime hero is removed, the Last Star receives seven reliable youthful local voice clips, gameplay music is handed into combat without the post-Prologue delay, every hostile field/lane/destination gains brighter two-pass bone-white filaments and motes, and Dawnward Aegis gains distinct radiating, pulsing, orbiting visuals across Ranks I–V and Awakened
...
the boss window. Gravestar gathers existing XP motes and Pulse Core primes 35
charge for the next valid enemy enclosure. Each campaign also has exactly two emergency Dawnheart windows:
