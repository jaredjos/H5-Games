...
describe('NIGHTTRACE campaign cinematics', () => {
  ...
  it('preserves the campaign reveal and the Bearer’s final choice in subtitle copy', () => {
    ...
    expect(allDialogue).toContain('You broke my body. The hunger is not flesh.')
    expect(allDialogue).toContain('That is only its echo, clinging to the Crown.')
    expect(allDialogue).toContain('There is no mouth left to hold the dawn.')
